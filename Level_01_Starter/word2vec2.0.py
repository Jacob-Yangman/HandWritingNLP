...
class MyModel():
    ...
    def forward(self, currWordOH, correspondOH, y):
        """
        ...
        self.hiddenLayer = self.W1[np.argmax(currWordOH)].reshape(1, -1)  # (1, embLen)
        predict = self.hiddenLayer @ (self.W2[:, np.argmax(correspondOH)].reshape(-1, 1))  # (1, 1)
    ...
    def backward(self):
        ...
        deltaHidden = G @ self.W2[:, np.argmax(self.y)].reshape(-1, 1).T  # (n, embLen)
