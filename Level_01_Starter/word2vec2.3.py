...
优化日志：不再获取全局triples，按照每个词采集正负样本，构建二元组，二元组中直接使用词的index
...
class MyModel():
    ...
    def forward(self, currWordIdx, correspondIdx, label):
        ...
        loss = -np.mean(label * np.log(self.proba) + (1 - label) * np.log(1 - self.proba))
        return loss

    def backward(self):
        G = deltaPred = self.proba * (1 - self.proba)
        deltaW2 = self.hiddenLayer.T @ G  # (embLen, 1)
        deltaHidden = G @ self.W2[:, np.argmax(self.label)].reshape(-1, 1).T  # (1, embLen)
...
def getCurrTriples(currId, lineWords):
    ...
    for nei in neighbors:
        triples.append((words2Idx[nei], 1))
    ...
    for neg in negativeSamples:
        triples.append((words2Idx[neg], 0))
    ...
    for e in range(epoch):
        ...
        for currLineWords in tqdm(allWords, desc="Training..."):
            ...
            for currId in range(0, len(currLineWords), step):
                ...
                currWordTriples = getCurrTriples(currId, currLineWords)

                for tri in currWordTriples:
                    loss = model.forward(currWordIdx, *tri)
                    model.backward()
        print(f"\nLoss >> {loss}")
    saveEmbs()
