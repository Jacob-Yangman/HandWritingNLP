...
class MyDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # MLM策略:从Index中随机取15%位置，其中80%替换为MASK，10%随机换成任意一个Index，剩余10%保持不变
        sampleNum = int(len(inputTrueDataIdx) * 0.15)
        sampleLocalIdx = sorted(random.sample([i for i in range(len(inputTrueDataIdx))], sampleNum + 3))

        # maskNum = int(sampleNum * 0.8)
